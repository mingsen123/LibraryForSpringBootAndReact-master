 ...
 */
@Service
public class BorrowSerciceImpl implements BookBorrowService {
    @Autowired
    ...
    public ResultDTO doBorrow(int bookId, Date startDate, Date endDate, int borrowCount, int userId, String userName, boolean vipFlag) {
        ...
        if(bookCount <= 0){
            //TODO 在查找书籍的时候如果返回的书籍数量为0，前端直接禁用租借按钮，同时加一个效果，当前书籍已经租光了！
            return new ResultDTO(HttpCode.FAIL.getCode(), "当前书籍已全部租借，请看看别的书吧");
        }

        if(borrowCount > bookCount){
            return new ResultDTO(HttpCode.FAIL.getCode(), "借阅数量超过书籍存量，当前书籍存量："+ bookCount);
        }
        //Step 3:真正的借书操作
